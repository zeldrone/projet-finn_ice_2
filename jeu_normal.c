...
#include <winalleg.h>
void objets(int *emplacement, int *precedent, int* potion, char tab[15][19], int changement [50][2], int direction_x, int direction_y, char affiche[15][19])
{
    int i,j;
    int buffer,buffer2;
    switch (*precedent)
    ...
    if (*precedent>100)
    {
        ...
        for(i=0; i<15; i++)
        {
            for(j=0; j<19; j++)
            {
                ...
                if ((tab[i][j]==*precedent+10)||(tab[i][j]==*precedent-10))
                {
                    tab[i][j]=2;
                    tab[changement[*emplacement-1][1]][changement[*emplacement-1][0]]=-1;
...
void intitialisation_rec(FILE* niveau_rec, char tab[15][19])
{
    ...
    for(i=0; i<15; i++)
    {
        for(j=0; j<19; j++)
        {
            fscanf(niveau_rec, "%d", (int*)&tab[i][j]);
        }
    }
}
...
void initialisation_console(char tab[15][19], char affiche_console[15][19], int score, int score_niveau)
{
    ...
    for(i=0; i<15; i++)
    {
        ...
        if (i==1) printf("score niveau: %d", score_niveau);
        printf("\n");
    }
...
void deplacement(char tab[15][19], int *clef,int score_general, int* score, int *precedent, int changement[50][2], int* emplacement, int* potion, char affiche[15][19], int niveau)
{
    int i,j,x,y,z=1;
    char move;
    int test=0;
    ...
    if (kbhit())
    {
        ...
        *emplacement= *emplacement +1;
        ...
        if (test)
        {
            ...
            objets(emplacement, precedent, potion, tab, changement, direction_x, direction_y, affiche);
        }
